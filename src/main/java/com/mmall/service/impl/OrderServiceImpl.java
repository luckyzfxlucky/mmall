...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId , Integer shippingId){
        ...
        BigDecimal payment = this.getOrderTotlePrice(orderItemList);

        //生成订单
        Order order = this.assembleOrder(userId,shippingId,payment);
        if(order == null){
            return ServerResponse.createdByErrorMsg("生成订单错误");
        }
        if(CollectionUtils.isEmpty(orderItemList)){
            return ServerResponse.createdByErrorMsg("购物车为空");
        }
    ...
    private ServerResponse<List<OrderItem>> getCartOrderItem(Integer userId , List<Cart> cartList){
        ...
        for(Cart cartItem : cartList){
            ...
            if(Const.ProductStatus.ON_SALE.getCode() != product.getStatus()){
                return ServerResponse.createdByErrorMsg("产品"+product.getName()+"不是在线售卖状态！");
            }
            //校验库存
            if(cartItem.getQuantity() > product.getStock()){
                return ServerResponse.createdByErrorMsg("产品"+product.getName()+"库存不足！");
            }
    ...
    public ServerResponse pay(Long orderNo , Integer userId , String path) {
        ...
        if (order == null) {
            return ServerResponse.createdByErrorMsg("用户没有该订单！");
        }
        ...
        String subject = new StringBuilder().append("mmall扫码支付，订单编号：").append(outTradeNo).toString();
        ...
        switch (result.getTradeStatus()) {
            ...
            case FAILED:
                log.error("支付宝预下单失败!!!");
                return ServerResponse.createdByErrorMsg("支付宝预下单失败!!!");

            case UNKNOWN:
                log.error("系统异常，预下单状态未知!!!");
                return ServerResponse.createdByErrorMsg("系统异常，预下单状态未知!!!");

            default:
                log.error("不支持的交易状态，交易返回异常!!!");
                return ServerResponse.createdByErrorMsg("不支持的交易状态，交易返回异常!!!");
        }
    }
    // 简单打印应答
    ...
    public ServerResponse aliCallback(Map<String,String> params){
        ...
        if(order == null){
            return ServerResponse.createdByErrorMsg("非mmall的订单，回调请忽略！");
        }
        if(order.getStatus() >= Const.OrderStatusEnum.PAID.getCode()){
            return ServerResponse.createdBySuccess("支付宝重复调用！");
        }
    ...
    public ServerResponse<String> cancle(Integer userId , Long orderNo){
        Order order = orderMapper.selectOrderByOrderNoAndUserId(orderNo, userId);
        if(order == null){
            return ServerResponse.createdByErrorMsg("该用户没有此订单！");
        }
        if(order.getStatus() != Const.OrderStatusEnum.NO_PAY.getCode()){
            return ServerResponse.createdByErrorMsg("已付款，无法取消订单!");
        }
        Order order1 = new Order();
        order1.setId(order.getId());
        order1.setStatus(Const.OrderStatusEnum.CANCELED.getCode());
        int rowCount = orderMapper.updateByPrimaryKeySelective(order1);
        if(rowCount >0){
            return ServerResponse.createdBySuccess();
        }
        return ServerResponse.createdByError();
    ...
    public ServerResponse<OrderVo> getOrderDetail(Integer userId , Long orderNo){
        ...
        return  ServerResponse.createdByErrorMsg("没有找到该订单");
    }
    ...
    public ServerResponse<OrderVo> manageDetail(Long orderNo){
        ...
        return ServerResponse.createdByErrorMsg("订单不存在");
    }
